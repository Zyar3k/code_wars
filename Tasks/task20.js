...
// CODE WARS FAIL
function isInteresting(number, awesomePhrases) {
  const interestingNumber = 2;
  const almostInterestingNumber = 1;
  const notInterestingNumber = 0;

  const str = number.toString(),
    isZeros = (num) => /^[1-9](0+)$/.test(num),
    isPalindrome = (num) => num.split("").reverse().join("") === num,
    isAwesomePhrases = (num) => awesomePhrases.includes(num),
    isSeqIncrease = (num) =>
    ...
    isClose = (num) => {
      ...
      for (let i = 0; i < awesomePhrases.length; i++) {
        ...
        if (num >= element - 2 && num <= element + 2) {
          return true;
        }
      }
      for (let i = 0; i < check; i++) {
        const next = minNum + i;
        const anotherString = (minNum + i).toString();
        if (
          isZeros(anotherString) ||
          isPalindrome(anotherString) ||
          isSeqDecrease(anotherString) ||
          isSeqIncrease(anotherString) ||
          isAwesomePhrases(next)
        ) {
          return true;
        }
      }
  ...
  if (
    ...
    return almostInterestingNumber;
  }
  return notInterestingNumber;
}
